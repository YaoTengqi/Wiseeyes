...
class FPGAManager(object):
    ...
    def find_sink_op(self, graph):
        for tname, t in graph.tensor_registry.items():
            if len(t.output_nodes) == 0 or ((t.output_nodes[0].name[4] > tname[4] or t.output_nodes[0].name[5] > tname[5])  and 'upsample' not in t.output_nodes[0].name and 'pool' not in t.output_nodes[0].name) or 'concat' in tname or 'upsample' in tname or 'concat' in tname or 'split' in tname or len(t.output_nodes) > 1:
...
#        print str(self.output_t.name) + " " + str(self.output_t.dtype.frac_bits)
    ...
    def initialize_graph(self, graph, array_m, array_n):
        ...
        self.fpga_memspace.write('ddr', 0x51000000, np.zeros((1<<30), dtype=np.int8))
