...
class TypeCastOp(NodeOp):
    def __init__(self, data, out_scale, output_dtype, node_name=None):
        self.data = data
        input_tensors = data
        self.output_dtype = output_dtype
        self.o_scale = out_scale
...
class Convolution(NodeOp):
    def __init__(self, data, weights, bias, node_name, pad='SAME', stride=None, group=1, dtype=FQDtype.FP32):
        ...
        assert self.weights.shape[-1] == input_channels, 'Expected {} input channels in weights, got {}'.format(
        ...
        assert bias.shape[0] == weights.shape[-4], 'Bias shape {} does not match weights shape {}'.format(bias.shape,
                                                                                                          weights.shape)
    ...
    def _get_output_scale(self):
        return self.data.scale * self.weights.scale
    ...
    def _autograd(self, x, y, grad_dtype=FQDtype.FP32):
        ...
        if x == self.data:
            if self.input_loss[0] is None:
                op = ConvolutionBackprop(data=self.data, weights=self.weights, output_loss=self.output_loss,
                                         pad=self.pad, stride=self.stride, group=self.group, node_name=self.name,
                                         dtype=grad_dtype)
                self.input_loss[0] = op.output_tensors
            return self.input_loss[0]
        else:
            if self.input_loss[1] is None:
                op = ConvolutionGradient(data=self.data, weights=self.weights, output_loss=self.output_loss,
                                         pad=self.pad, stride=self.stride, group=self.group, node_name=self.name,
                                         dtype=grad_dtype)
...
class UpSampling(NodeOp):
    def __init__(self, data, sample_kernel, node_name, dtype=None):

        # Input data >3D
        self.data = data

        # Pooling kernel
        assert len(sample_kernel) == len(data.shape)
        ...
        self.dtype = dtype
        super(UpSampling, self).__init__(node_name=node_name, input_tensors=input_tensors)

    def _get_output_shape(self):
        cout = self.data.shape[-1]
        hout = self.data.shape[-3] * self.sample_kernel[-3]
        wout = self.data.shape[-2] * self.sample_kernel[-2]
        out_shape = []
        for i in range(len(self.data.shape) - 3):
            out_shape.append(self.data.shape[i])
        out_shape.append(hout)
    ...
    def get_ops(self):
        ...
        hfil = self.sample_kernel[-2]
        wfil = self.sample_kernel[-1]
...
class MaxPooling(NodeOp):
    ...
    def _get_output_shape(self):
        cout = self.data.shape[-1]
        hout = (self.data.shape[-3] - self.pooling_kernel[-3] + self.pad[-3][0] + self.pad[-3][1]) // self.stride[
            -3] + 1
        wout = (self.data.shape[-2] - self.pooling_kernel[-2] + self.pad[-2][0] + self.pad[-2][1]) // self.stride[
            -2] + 1
        out_shape = []
        for i in range(len(self.data.shape) - 3):
            out_shape.append(self.data.shape[i])
    ...
    def _autograd(self, x, y, grad_dtype=FQDtype.FP32):
        ...
        if self.input_loss[0] is None:
            op = MaxPoolBackprop(data=self.data, pooling_kernel=self.pooling_kernel, output_loss=self.output_loss,
                                 node_name=self.name)
            self.input_loss[0] = op.output_tensors
...
class Split(NodeOp):
    def __init__(self, data, node_name, dtype=None):
        ...
        self.dtype = dtype
        super(Split, self).__init__(node_name=node_name, input_tensors=input_tensors)

    def _get_output_shape(self):
        out_shape = list(self.data.shape)
        out_shape[-1] = int(out_shape[-1] / 2)
        return tuple(out_shape)
    ...
    def __init__(self, data, concat_dim, node_name, dtype=None):
        ...
        for _data in data:
            ...
            for dim in range(len(_data.shape)):
                if dim != concat_dim:
                    assert _data.shape[dim] == data[0].shape[dim], '{} does not match {} for dimension {}'.format(
                        data[0].__str__(), _data.__str__(), dim)
        ...
        for i in range(len(self.data)):
            if x == self.data[i]:
                if self.input_loss[i] is None:
                    op = ConcatBackprop(data=self.data[i], output_loss=self.output_loss, node_name=self.name,
                                        dtype=grad_dtype)
...
class Add(NodeOp):
    ...
    def _autograd(self, x, y, grad_dtype=FQDtype.FP32):
        ...
        for i in range(len(self.data)):
            if x == self.data[i]:
                if self.input_loss[i] is None:
                    op = AddBackprop(data=self.data[i], output_loss=self.output_loss, node_name=self.name,
                                     dtype=grad_dtype)
                    self.input_loss[i] = op.output_tensors
                return self.input_loss[i]
...
class MatMul(NodeOp):
    def __init__(self, data, weights, biases, name, dtype=None):
        ...
        assert self.weights.shape[-1] == self.data.shape[
            -1], 'Dimension mismatch between data ({}) and weights ({})'.format(self.data, self.weights)
    ...
    def _autograd(self, x, y, grad_dtype=FQDtype.FP32):
        ...
        if x == self.data:
            if self.input_loss[0] is None:
                op = MatMulBackprop(data=self.data, weights=self.weights, output_loss=self.output_loss,
                                    node_name=self.name, dtype=grad_dtype)
                self.input_loss[0] = op.output_tensors
            return self.input_loss[0]
        else:
            if self.input_loss[1] is None:
                op = MatMulGradient(data=self.data, weights=self.weights, output_loss=self.output_loss,
                                    node_name=self.name, dtype=grad_dtype)
                self.input_loss[1] = op.output_tensors
...
class AddBias(NodeOp):
    ...
    def _autograd(self, x, y, grad_dtype=FQDtype.FP32):
        ...
        if x == self.data:
            if self.input_loss[0] is None:
                op = AddBiasBackprop(data=self.data, weights=self.weights, output_loss=self.output_loss, dim=self.dim,
                                     node_name=self.name, dtype=grad_dtype)
                self.input_loss[0] = op.output_tensors
            return self.input_loss[0]
        else:
            if self.input_loss[1] is None:
                op = AddBiasGradient(data=self.data, weights=self.weights, output_loss=self.output_loss, dim=self.dim,
                                     node_name=self.name, dtype=grad_dtype)
                self.input_loss[1] = op.output_tensors
            return self.input_loss[1]
...
def typecast(i, scale, dtype=FQDtype.FXP8, name=None):
    if scale is None or i.scale == scale:
        return i
    else:
        return TypeCastOp(i, scale, dtype).output_tensors
...
def conv2D(i, w, b, name=None, stride=None, pad='SAME', group=1, dtype=None, out_scale=1.0):
    g = get_default_graph()
    op = Convolution(i, w, b, name, stride=stride, pad=pad, group=group, dtype=dtype)
    op.output_tensors.ddr_vilid = True
    return typecast(op.output_tensors, out_scale, dtype)


def upSampling(i, sample_kernel, name=None, dtype=None, out_scale = None):
    g = get_default_graph()
    op = UpSampling(i, sample_kernel, name, dtype)
    return typecast(op.output_tensors, out_scale, dtype)


def maxPool(i, pooling_kernel, stride=(1, 2, 2, 1), pad='VALID', name=None, dtype=None, out_scale = None):
    g = get_default_graph()
...
def concat(data, concat_dim, name=None, dtype=None, out_scale=None):
    op = Concat(data, concat_dim, name, dtype=dtype)
    return typecast(op.output_tensors, out_scale, dtype)


def split(data, name=None, dtype=None, out_scale = None):
    op = Split(data, name, dtype=dtype)
    return typecast(op.output_tensors, out_scale, dtype)
...
def leakyReLU(data, name=None, alpha=0.1, dtype=FQDtype.FXP8, out_scale=0.1):
