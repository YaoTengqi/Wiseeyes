...
from dnnweaver2.tensorOps.cnn import conv2D, maxPool, flatten, matmul, addBias, batch_norm, \
    reorg, concat, leakyReLU, upSampling, split
...
def yolo_convolution(tensor_in, filters=32, kernel_size=3,
                     ...
                     weight_scale=1.0, out_scale=1.0, out_z=0,
                     stride=(1, 1, 1, 1)):
...
def get_graph(train=False):
    g = Graph('Face_Recognition-Test: 8-bit', dataset='imagenet', log_level=logging.INFO)
    batch_size = 1
    # data_scale = list(np.load('/home/lab04/pycharmProject/0112_dpet/dnnweaver2/benchmarks/face_rec_input_scale.npy'))
    # weight_scale = list(np.load('/home/lab04/pycharmProject/0112_dpet/dnnweaver2/benchmarks/face_rec_weight_scale.npy'))
    data_scale = list(np.load('../../dnnweaver2/benchmarks/face_rec_input_scale.npy'))
    weight_scale = list(np.load('../../dnnweaver2/benchmarks/face_rec_weight_scale.npy'))
    # print(data_scale)
    # print(weight_scale)
    with g.as_default():
        with g.name_scope('inputs'):
            input0 = get_tensor(shape=(batch_size, 128, 128, 3), name='data', dtype=FQDtype.FXP8, trainable=False,
                                scale=data_scale[0])

        with g.name_scope('conv0'):
            layer0 = yolo_convolution(input0, filters=32, kernel_size=3,
        ...
        with g.name_scope('conv1'):
            layer1 = yolo_convolution(layer0, filters=64, kernel_size=3,
                                      batch_normalize=False, act='ReLU',
                                      weight_scale=weight_scale[1], out_scale=data_scale[2],
                                      stride=(1, 2, 2, 1))

        # start resblock_body1
        ...
        with g.name_scope('concat5'):
            layer25 = concat((layer18, layer23), 3)
        # end resblock_body3

        with g.name_scope('conv14'):
            layer26 = yolo_convolution(layer25, filters=256, kernel_size=3,
                                       batch_normalize=False, act='ReLU',
                                       weight_scale=weight_scale[14], out_scale=data_scale[15])
        with g.name_scope('conv15'):
            layer27 = yolo_convolution(layer26, filters=128, kernel_size=3,
                                       batch_normalize=False, act='ReLU',
                                       weight_scale=weight_scale[15], out_scale=data_scale[16])

        with g.name_scope('conv16'):
            layer28 = yolo_convolution(layer27, filters=64, kernel_size=3,
                                       batch_normalize=False, act='ReLU',
                                       weight_scale=weight_scale[16], out_scale=data_scale[17],
                                       stride=(1, 2, 2, 1))

        with g.name_scope('flatten0'):
            layer29 = flatten(layer28)

        with g.name_scope('fc_conv'):
            layer30 = yolo_convolution(layer29, filters=512, kernel_size=1,
                                       batch_normalize=False, act='linear',
                                       weight_scale=weight_scale[17], out_scale=data_scale[18], out_z=-8)
