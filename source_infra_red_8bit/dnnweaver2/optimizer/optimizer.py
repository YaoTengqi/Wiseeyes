...
# tile_deps['KW/kw'] = {'ibuf': True, 'wbuf': True,  'obuf': False,  'bbuf': False}
# tile_deps['Kh/kh'] = {'ibuf': True, 'wbuf': True,  'obuf': False,  'bbuf': False}
...
def get_stats_fast(conv_params, tiling, order_type, verbose=False):
    ...
    """
    acc_obj, K, O,O_h, S, IC, OC, B, iprec, wprec, im2col, energy_cost, _, _ = conv_params
    ...
    writes['ibuf'] = iw * ih * ceil_a_by_b(ic, acc_obj.N) * acc_obj.N * b * iprec * 2

    bprec = 16
    writes['bbuf'] = ceil_a_by_b(oc, acc_obj.M) * acc_obj.M * bprec

    oprec = 32
    writes['obuf'] = ow * oh * ceil_a_by_b(oc, acc_obj.M) * acc_obj.M * b * oprec * 2
    reads['obuf'] = ow * oh * ceil_a_by_b(oc, acc_obj.M) * acc_obj.M * b * oprec * 2
    # writes['obuf'] = 0
    # reads['obuf'] = 0
    ...
    stats.reads['ibuf'] += num_tiles * (kw * kh * ic * oc) * (b * iw * ih) * iprec * 2
    stats.reads['obuf'] += num_tiles * (kw * kh * ic * oc) * (b * ow * oh) * oprec * 2
    stats.writes['obuf'] += num_tiles * (kw * kh * ic * oc) * (b * ow * oh) * oprec * 2
...
def optimize_for_order(conv_params, pool_kernel=None, pool_stride=None, sequential=True, first_layer=False):
    # Generate permutations for the order
    loops = ['B/b', 'OW/ow', 'OH/oh', 'IC/ic', 'OC/oc']
    order = set(permutations(loops))
    return_dict = {}
    acc_obj, K, O, O_h, S, IC, OC, B, iprec, wprec, im2col, energy_cost = conv_params
    ...
    if pool_stride is None:
        pool_stride = (1, 1, 1, 1)
    conv_params_with_pool = acc_obj, K, O, O_h, S, IC, OC, B, iprec, wprec, im2col, energy_cost, pool_kernel, pool_stride
    lock = Manager().Lock()
    q = Manager().Queue()
    q.put(0)
    if not sequential:
        
        _bound_optimizer_method = functools.partial(_optimize_for_order, conv_params_with_pool,first_layer,lock,q)
        ...
        best_order = None
        for o in order:
            tiling, order_type, cycles, energy, _, _ = _optimize_for_order(conv_params_with_pool, first_layer,lock.Value,o)
...
def _optimize_for_order(conv_params, first_layer,lock,q,order_type, verbose=False):
    """
    ...
    """
    acc_obj, K, O, O_h, S, IC, OC, B, iprec, wprec, im2col, energy_cost, pool_kernel, pool_stride = conv_params
    I = (O - 1) * S + K
    pool_O = (O - pool_kernel[2]) / pool_stride[2] + 1  #该变量的由来？
    pool_Oh = (O_h - pool_kernel[1]) / pool_stride[1] + 1  # 该变量的由来？
    ...
    for _b in range(num_B_tiles):
        ...
        #make sure tiling > 6
        for p_ow in range(1, int(pool_O)+1):
            if pool_O % p_ow != 0:
                continue
           # p_oh = p_ow
            if first_layer:
                p_ow = 8
            ow = (p_ow-1) * pool_stride[2] + pool_kernel[2]
          ...
          #  num_oh = ceil_a_by_b(pool_O, p_oh)
            ...
            # if ow < 6 or ow > 16:
            #     continue
            for p_oh in range(1, int(pool_Oh)+1):
                if pool_Oh % p_oh != 0:
                    continue
                if first_layer:
                    p_oh = 8
                oh = (p_oh - 1) * pool_stride[1] + pool_kernel[1]
               
                num_oh = ceil_a_by_b(pool_Oh, p_oh)
                if num_oh * p_oh != pool_Oh:
                    continue
                if num_oh == 1:
                    oh = int(math.ceil(oh/2))
                elif num_oh % 2 == 0:
                    num_oh = int(num_oh / 2)
                else:
                    continue
                oh += int(oh % pool_stride[1])
                if ow * oh < 32:
                    continue
                for _ic in range(1):
                    ic = 32
    ...
    #                     print(tiling)
                        ...
                        # if best_energy is None or best_energy > energy or (best_energy == energy and best_cycles > cycles):
                            ...
                            if best_c == 0:
                               q.put(best_cycles)
                               print(order_type,"best_tiling:",best_tiling, "best_cycles:",best_cycles)
                            elif best_cycles < best_c:
                                q.put(best_cycles)
                                print(order_type,"best_tiling:",best_tiling, "best_cycles:",best_cycles)
                            else:
                                q.put(best_c)
