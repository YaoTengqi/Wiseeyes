...
def get_graph(train=False):
    ...
    data_scale = list(np.load('../../dnnweaver2/benchmarks/yolo_tiny_VOC_input_scale.npy'))
    print(data_scale)
    weight_scale = list(np.load('../../dnnweaver2/benchmarks/yolo_tiny_VOC_weight_scale.npy'))
    with g.as_default():
        ...
        with g.name_scope('conv17'):
            layer29 = yolo_convolution(layer28, filters=30, kernel_size=1,
                    batch_normalize=False, act='linear',
                    weight_scale=weight_scale[17], out_scale=data_scale[18], out_z = 206-128)
        ...
        with g.name_scope('conv20'):
            layer36 = yolo_convolution(layer35, filters=30, kernel_size=1,
                    batch_normalize=False, act='linear',
                    weight_scale=weight_scale[20], out_scale=data_scale[21], out_z = 216-128)
    return g
