...
def get_graph(train=False):
    ...
    data_scale = list(np.load('../../dnnweaver2/benchmarks/input_scale.npy'))
    weight_scale = list(np.load('../../dnnweaver2/benchmarks/weight_scale.npy'))
    with g.as_default():
        ...
        with g.name_scope('conv17'):
            layer29 = yolo_convolution(layer28, filters=75, kernel_size=1,
                    batch_normalize=False, act='linear',
                    weight_scale=weight_scale[17], out_scale=data_scale[18], out_z = 39)
        ...
        with g.name_scope('conv20'):
            layer36 = yolo_convolution(layer35, filters=75, kernel_size=1,
                    batch_normalize=False, act='linear',
                    weight_scale=weight_scale[20], out_scale=data_scale[21], out_z = 53)
    return g
