...
import array
import codecs
from dnnweaver2.fpga.MemoryModule import *
...
class FPGAMemSpace(object):
    ...
    def write(self, namespace, addr, data):
        #if namespace == 'ddr':self.log.debug('write,namespace={}, addr={}, data.type={},data={}'.format(namespace, addr, type(data), data))
        self.log.debug('write,namespace={}, addr={}, data.type={},data={}'.format(namespace, addr, type(data), data))
        assert namespace in ('pci_cl_data', 'pci_cl_ctrl', 'ddr')
        if self.sim_flag == 1:
            self.mem.write(namespace, addr, data)
            return None
    ...
    def read(self, namespace, addr, size=None):
        self.log.debug('debug:read,namespace={}, addr={}, size={}'.format(namespace, addr, size))
        assert namespace in ('pci_cl_data', 'pci_cl_ctrl', 'ddr')
        if self.sim_flag == 1:
            data = self.mem.read(namespace, addr, size)
            return data
        if namespace == 'pci_cl_ctrl':
            self.pci_cl_ctrl_mmap.seek(addr)
