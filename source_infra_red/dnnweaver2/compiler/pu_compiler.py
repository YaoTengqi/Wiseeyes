...
class PUCompiler(object):
    ...
    def compile_layer(self, conv_tiling, conv_out_tensor, pu_ops, simd_lanes=4):
        ...
        pu_inst_list.append(Pad_lines_Instruction(0,0,0))
        if branch_required:
        ...
        for loop, it in conv_tiling.items():
            if it[0] > 1 and loop in _pool_tile:
                ...
                if branch_required:
                    ...
                    stride = int(np.prod(shape[dim + 1:]) * dim_stride * 2 * simd_lanes) * branch_tile[loop]
                    if stride > (1 << 15):
        ...
        for idx in range(pool_kw * pool_kh):
            ...
            for op in pre_pool_ops:
                ...
                if isinstance(op, LeakyReLU):#actual ReLU
                    ...
                    #m = s1 / s2
                    #n = 12
                    #m0 = int(np.ceil(m * pow(2, n)))
                    m0=1
                    #shift = op.data.dtype.frac_bits - op.output_tensors.dtype.frac_bits
                    rs_data_reg = op.data.dtype.frac_bits - op.output_tensors.dtype.frac_bits
                    print(rs_data_reg)
                    print("TC END")
                    compute_instructions.append(ComputeMulImm(src0_addr=8, imm=m0, dest_addr=dest_reg))
        ...
        #tmp_reg = self.acquire_reg()
        #pu_inst_list.append(ComputeRshiftImm(src0_addr=tmp_reg, imm=rs_data_reg, dest_addr=tmp_reg))
        #self.release_reg(tmp_reg)
