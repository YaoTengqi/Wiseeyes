...
def get_graph(train=False):
    ...
    with g.as_default():
            ...
            with g.name_scope('inputs'):
                i = get_tensor(shape=(batch_size, 416,416,3), name='data', dtype=FixedPoint(8, 6), trainable=False)

            with g.name_scope('conv0'):
                layer0 = yolo_convolution(i, filters=32, kernel_size=3,
                        batch_normalize=True, act='leakyReLU',
                        w_dtype=FixedPoint(8, 6), c_dtype=FixedPoint(8, 4),
                        s_dtype=FixedPoint(8, 3), bn_dtype=FixedPoint(8, 5), stride=(1, 2, 2, 1))
