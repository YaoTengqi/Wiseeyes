...
Fpga_memspace_array = [0x41000000,0x51000000,0x57000000]
Compile_target = 0
...
class FPGAManager(object):
    def __init__(self,
        ...
        self.output_t = None
        self.output_ts = []
        self.entries_f = open('./data_yolo_v2/entries.txt', 'w')
    ...
    def send_input_nparr(self, input_nparr):
        ...
        print("1111")
        print(padded_data[0,350:,4:7,:])
        self.fpga_memspace.write('ddr', tin.fpga_addr, padded_data)
        np.save('./data_yolo_v2/input%d.npy' % tin.fpga_addr, padded_data)
        self.log.debug('tensor data: \n{}'.format(tin.data))

    # TODO: this is not a general impl. Needs to be cleaned up after hotchips.
    def find_sink_op(self, graph):
        for tname, t in graph.tensor_registry.items():
            if len(t.output_nodes) == 0 or ((t.output_nodes[0].name[4] > tname[4] or t.output_nodes[0].name[5] > tname[5])   and 'upsample' not in t.output_nodes[0].name) or 'concat' in tname or 'upsample' in tname or 'concat' in tname or 'split' in tname or len(t.output_nodes) > 1:
                self.output_ts.append(t)

    def get_tout_scale(self, index):
#        print str(self.output_t.name) + " " + str(self.output_t.dtype.frac_bits)
        return self.output_ts[index].scale
    
    def get_tout_zero_point(self, index):
#        print str(self.output_t.name) + " " + str(self.output_t.dtype.frac_bits)
        return self.output_ts[index].zero_point
    ...
    def recv_output_nparr(self):
        got_out_fpgas = []
        for i in range(len(self.output_ts)):
            t = self.output_ts[i]
            op = t.op
            ...
            con_fpga = list(t.fpga_shape)
            pad_len = np.prod(con_fpga[:-1]) * t.concat_c 
            con_fpga[-1] += t.concat_c
            print(t.fpga_size_in_bytes + pad_len)
            if "TypeCastOp" in t.name:
                got_out_fpga = np.array(array.array('b', self.fpga_memspace.read('ddr', t.fpga_addr, t.fpga_size_in_bytes + pad_len)), dtype=np.int8).reshape(tuple(con_fpga))
            else:
                got_out_fpga = np.array(array.array('B', self.fpga_memspace.read('ddr', t.fpga_addr, t.fpga_size_in_bytes + pad_len)), dtype=np.uint8).reshape(tuple(con_fpga))
            np.save('./data_yolo_v2/output%d.npy' % t.fpga_addr, got_out_fpga)
            got_out_fpga = self._unpad_tensor(t, got_out_fpga)
            got_out_fpgas.append([op.name ,got_out_fpga])
        return got_out_fpgas
    ...
    def initialize_graph_tensors(self, graph):
        ...
        for tname, t in graph.tensor_registry.items():
            ...
            self.log.debug('Tensor initialized with data in addr:{} \n{}'.format(t.fpga_addr,t.data))
    ...
    def initialize_graph(self, graph, array_m, array_n):
        ...
        self.fpga_memspace.write('ddr', Fpga_memspace_array[Compile_target], np.zeros((1<<30), dtype=np.int8))
        print("#########################################################################################################")
        print(Fpga_memspace_array[Compile_target])
        ...
        for opname, op in graph.op_registry.items():
            if isinstance(op, Convolution):
                ...
                if op.data.op is None:
                    ...
                    self.fpga_memspace.write('ddr', tin.fpga_addr, padded_data)
                    self.log.debug('tensor data: \n{}'.format(tin.data))
                    # self.write_data(padded_data, True)
                else:
                    # Need zero-padding for inputs
                    op.data.data = np.zeros(op.data.shape, dtype=np.int8)
                    self.fpga_memspace.write('ddr', op.data.fpga_addr, _pad_tensor(op.data))
                    # self.write_data(_pad_tensor(op.data), True)
                ...
                # self.write_data(tbias_ddr, False)
                ...
                if padding > 0:
                    ...
                    neg = np.ones(op.output_tensors.fpga_shape, dtype=np.int32) * -1 * (1<<31)
                    self.fpga_memspace.write('ddr', op.output_tensors.fpga_addr, neg)
                    # self.write_data(neg, True)
                else:
                    self.fpga_memspace.write('ddr', op.output_tensors.fpga_addr, np.zeros(op.output_tensors.fpga_size, dtype=np.int32))
                        # self.write_data(np.zeros(op.output_tensors.fpga_size, dtype=np.int64), True)
                ...
                self.log.debug('tensor data: \n{}'.format(scale.data))
                # self.write_data(mean_ddr, False)
                # self.write_data(scale_ddr, False)

    def save_graph(self, graph, array_m, array_n):
        self.log.info('Saving graph: {}'.format(graph.name))
        self.find_sink_op(graph)
        for opname, op in graph.op_registry.items():
            if isinstance(op, Convolution):
                # data
                if op.data.op is None:
                    self.input_op = op
                    tin = op.data
                    self.log.debug('Saving tensor {} to fpga addr {}'.format(op.data, tin.fpga_addr))
                    pad = op.pad
                    b, I, _, ic = tin.fpga_shape
                    padded_data = _pad_tensor(tin)

                    # self.save_graph_entry('{}'.format(op.data), tin.fpga_addr, padded_data)
                    self.log.debug('tensor data: \n{}'.format(tin.data))
                else:
                    # Need zero-padding for inputs
                    op.data.data = np.zeros(op.data.shape, dtype=np.int8)
                    #self.save_graph_entry('{}'.format(op.data), op.data.fpga_addr, _pad_tensor(op.data))

                # weights
                tw = op.weights
                self.log.debug('Saving tensor {} to fpga addr {}'.format(tw, tw.fpga_addr))
                oc, kh, kw, ic = tw.fpga_shape
                assert oc % array_m == 0
                tw_data = _pad_tensor(tw).reshape(int(oc/array_m),array_m,kh,kw,ic)
                tw_ddr = np.transpose(tw_data, (0,2,3,4,1)).copy()
                # self.fpga_memspace.write('ddr', tw.fpga_addr, tw_ddr)
                self.save_graph_entry('{}'.format(tw), tw.fpga_addr, tw_ddr)
                self.log.debug('tensor data: \n{}'.format(tw_ddr))
                # self.write_data(tw_ddr, True)
                # bias
                tbias = op.bias
                self.log.debug('Saving tensor {} to fpga addr {}'.format(tbias, tbias.fpga_addr))
                self.log.debug('tensor data: \n{}'.format(tbias.data))
                tbias_ddr = np.pad(tbias.data, tbias.fpga_pad, 'constant', constant_values=(0,0))
                # self.fpga_memspace.write('ddr', tbias.fpga_addr, tbias_ddr)
                self.save_graph_entry('{}'.format(tbias), tbias.fpga_addr, tbias_ddr)
                # self.write_data(tbias_ddr, False)
                # Need negative padding for conv output
                padding = sum(x[0]+x[1] for x in op.output_tensors.fpga_pad)
                # if padding > 0:
                ...
                #     neg = np.ones(op.output_tensors.fpga_shape, dtype=np.int32) * -1 * (1<<31)
                #     # self.fpga_memspace.write('ddr', op.output_tensors.fpga_addr, neg)
                #     self.save_graph_entry('ddr', op.output_tensors.fpga_addr, neg)
                #     # self.write_data(neg, True)
                # else:
                #     # self.fpga_memspace.write('ddr', op.output_tensors.fpga_addr, np.zeros(op.output_tensors.fpga_size, dtype=np.int64))
                #     # self.write_data(np.zeros(op.output_tensors.fpga_size, dtype=np.int64), True)
                #     self.save_graph_entry('ddr', op.output_tensors.fpga_addr,
                #                              np.zeros(op.output_tensors.fpga_size, dtype=np.int32))

            elif isinstance(op, BatchNorm):
                mean = op.mean
                scale = op.scale
                self.log.debug('Saving tensor {} to fpga addr {}'.format(mean, mean.fpga_addr))
                mean_ddr = np_array_to_ddr(mean.data, [mean.size], [1])
                self.save_graph_entry('{}'.format(mean), mean.fpga_addr, mean_ddr)
                self.log.debug('tensor data: \n{}'.format(mean.data))
                self.log.debug('Saving tensor {} to fpga addr {}'.format(scale, scale.fpga_addr))
                scale_ddr = np_array_to_ddr(scale.data, [scale.size], [1])
                self.save_graph_entry('{}'.format(scale), scale.fpga_addr, scale_ddr)
                self.log.debug('tensor data: \n{}'.format(scale.data))
                # self.write_data(mean_ddr, False)
                # self.write_data(scale_ddr, False)

        self.entries_f.close()

    def save_graph_entry(self, entry_name, addr, data):
        # name = "./data_yolo_v2/{}{}.npy".format(entry_name.replace('/', '-').replace('[', '-').replace(']', '-')
        #                                 .replace('(', '').replace(')', ''), addr)
        name = ''
        if entry_name != 'ddr':
            name = './data_yolo_v2/{}{}.npy'.format(entry_name[0:9].replace('/', '-'), addr)
        else:
            name = './data_yolo_v2/{}{}.npy'.format(entry_name, addr)
        np.save(name, data)
        self.write_entry(addr, name)

    def write_entry(self, addr, name):
        self.entries_f.write('{} {}\n'.format(addr, name))
