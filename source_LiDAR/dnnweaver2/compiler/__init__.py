...
class FPGAMemoryManager(object):
    ...
    def alloc(self, tensor):
        ...
        if tensor.fpga_addr is None:
            ...
            # self.curr_ddr_ptr += int(math.ceil((tensor.fpga_size_in_bytes + np.prod(
            #     pad_fpga[:-1]) * tensor.concat_c * tensor.dtype.bits / 8.0) / 1024.) * 1024)
            if "data" in tensor.name:
                self.curr_ddr_ptr += 1024 * 1024* 32
            else:
...
class GraphCompiler(object):
    ...
    def optimize_tiling(self, op, graph, acc_obj, pool_kernel=None, pool_stride=None):
        ...
        S = op.stride[-2]
        O_h = op.output_tensors.fpga_shape[-3]
        ...
        conv_params = (acc_obj, K, O, O_h, S, IC, OC, B, iprec, wprec, im2col, energy_cost)
    ...
    def _alloc_tensor(self, graph):
        for tname, t in graph.tensor_registry.items():
            if isinstance(t, Tensor) and t.ddr_vilid:
    ...
    def _conv_compile(self, conv_op, pu_op, tiling, array_n, array_m, last=False):
        ...
        for l, it in tiling.items():
            if it[0] > 1:
                if conv_op.data.shape[-1] == 3 and (l == 'IC/ic' or l == 'OW/ow'):
                    continue
        ...
        padded_tile_shape_mapping = {
            ...
            ScratchPad.WBUF: (oc, kh, kw, ic * 32),
            ScratchPad.BIAS: (oc,)
        }
        ...
        inner_loop_order = ('IC/ic', 'OW/ow', 'OH/oh', 'KW/kw', 'KH/kh', 'OC/oc', 'B/b')
    ...
    def compile(self, graph, acc_obj):
        ...
        for i in range(len(macro_node_array)):
            ...
            if isinstance(op, Concat):
                ...
                fpga_pad1 = list(op.input_tensors[1].fpga_pad)
                max_pad = max(fpga_pad[1][0], fpga_pad0[1][0])
                max_pad = max(max_pad, fpga_pad1[1][0])
                max_pad = max(1, max_pad)

                # padding
                fpga_pad[1] = (max_pad, max_pad)
                fpga_pad[2] = (max_pad, max_pad)
                fpga_pad[3] = (0, int(math.ceil((con_ic + fpga_pad0[-1][1]) / float(array_n)) * array_n) - con_ic)
                fpga_pad0[1] = (max_pad, max_pad)
                fpga_pad0[2] = (max_pad, max_pad)
                fpga_pad1[1] = (max_pad, max_pad)
                fpga_pad1[2] = (max_pad, max_pad)
                op.output_tensors.fpga_pad = tuple(fpga_pad)
                op.input_tensors[0].fpga_pad = tuple(fpga_pad0)
                op.input_tensors[1].fpga_pad = tuple(fpga_pad1)

                self.fpga_manager.alloc(op.output_tensors)
                op.input_tensors[0].fpga_addr = op.output_tensors.fpga_addr
                op.input_tensors[0].concat_c = op.output_tensors.fpga_shape[-1] - op.input_tensors[0].fpga_shape[-1]
                op.input_tensors[0].post_c = op.output_tensors.fpga_shape[-1] - op.input_tensors[0].fpga_shape[-1]
                op.input_tensors[1].concat_c = op.output_tensors.fpga_shape[-1] - op.input_tensors[1].fpga_shape[-1]
                op.input_tensors[1].post_c = op.output_tensors.fpga_shape[-1] - op.input_tensors[1].fpga_shape[-1] - \
                                             op.input_tensors[0].shape[-1]
                op.input_tensors[1].fpga_addr = op.output_tensors.fpga_addr + int(
                    op.input_tensors[0].shape[-1] * op.input_tensors[0].dtype.bits / 8)
