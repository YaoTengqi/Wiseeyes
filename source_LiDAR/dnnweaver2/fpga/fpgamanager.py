...
class FPGAManager(object):
    ...
    def send_input_nparr_1(self, input_nparr):
        # data
        op = self.input_op
        op.data.data = input_nparr
        tin = op.data
        ...
        b, I, _, ic = tin.fpga_shape
        padded_data = _pad_tensor(tin)
        self.fpga_memspace.write('ddr', tin.fpga_addr+6291456, padded_data)
        print("!!!!!!!!!!!!!!!!!!!!!!! input address",tin.fpga_addr+6291456)
        np.save('./data_yolo_v2/input%d.npy' % tin.fpga_addr, padded_data)
        self.log.debug('tensor data: \n{}'.format(tin.data))
    ...
    def find_sink_op(self, graph):
        for tname, t in graph.tensor_registry.items():
            if len(t.output_nodes) == 0 or (t.output_nodes[0].name[4] > tname[4]   and 'upsample' not in t.output_nodes[0].name) or 'concat' in tname or 'upsample' in tname or 'concat' in tname or 'split' in tname or len(t.output_nodes) > 1:
                self.output_ts.append(t)

    def get_tout_frac_bits(self, index):
#        print str(self.output_t.name) + " " + str(self.output_t.dtype.frac_bits)
        return self.output_ts[index].dtype.frac_bits
    ...
    def recv_output_nparr(self):
        ...
        for i in range(len(self.output_ts)):
            ...
            pad_len = np.prod(con_fpga[:-1]) * t.concat_c * 2
            con_fpga[-1] += t.concat_c
            got_out_fpga = np.array(array.array('h', self.fpga_memspace.read('ddr', t.fpga_addr, t.fpga_size_in_bytes + pad_len)), dtype=np.int16).reshape(tuple(con_fpga))
            output_addr += [t.fpga_addr]
        ...
        path = './data_yolo_v2/output_addr.txt'
        f = open(path,'w',encoding='utf-8')
        f.write(str(output_addr))
        f.close()
    ...
    def initialize_graph(self, graph, array_m, array_n):
        ...
        self.fpga_memspace.write('ddr', 0x40000000, np.zeros((1<<30), dtype=np.int8))
            ...
            if isinstance(op, Convolution):
                ...
                if padding > 0:
                    ...
                    self.save_graph_entry('ddr', op.output_tensors.fpga_addr,
                                             np.zeros(op.output_tensors.fpga_size, dtype=np.int64))
    ...
    def save_graph_entry(self, entry_name, addr, data):
        ...
        if entry_name != 'ddr':
            ...
            name_entries = './{}{}.npy'.format(entry_name[0:9].replace('/', '-'), addr)
        else:
            name = './data_yolo_v2/{}{}.npy'.format(entry_name, addr)
            name_entries = './{}{}.npy'.format(entry_name, addr)
        np.save(name, data)
        self.entries_f.write('{} {}\n'.format(addr, name_entries))
...
#                sleep(0.0001)
    ...
    def input_offset1(self):
        self.fpga_memspace.write('pci_cl_ctrl', 120, 1)

    def input_offset2(self):
        self.fpga_memspace.write('pci_cl_ctrl', 120, 2)
